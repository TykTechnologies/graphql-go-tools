...
import (
	...
	"github.com/jensneuse/graphql-go-tools/pkg/engine/resolve/fastjson/fastfloat"
)
...
func escapeStringToWriter(writer io.Writer, s string) (err error) {
	// Fast path - nothing to escape.
	_, err = writer.Write(quote)
	_, err = writer.Write(s2b(s))
	_, err = writer.Write(quote)
	return err
}
...
func (o *Object) MarshalToWriter(writer io.Writer) (err error) {
	_, err = writer.Write(lBrace)
	for i, kv := range o.kvs {
		if o.keysUnescaped {
			err = escapeStringToWriter(writer, kv.k)
		} else {
			_, err = writer.Write(quote)
			_, err = writer.Write(s2b(kv.k))
			_, err = writer.Write(quote)
		}
		_, err = writer.Write(colon)
		err = kv.v.MarshalToWriter(writer)
		if i != len(o.kvs)-1 {
...
func (v *Value) MarshalToWriter(writer io.Writer) (err error) {
	switch v.t {
	case typeRawString:
		_, err = writer.Write(quote)
		_, err = writer.Write(s2b(v.s))
		_, err = writer.Write(quote)
	case TypeObject:
		err = v.o.MarshalToWriter(writer)
	case TypeArray:
		_, err = writer.Write(lBrack)
		for i, vv := range v.a {
			err = vv.MarshalToWriter(writer)
			if i != len(v.a)-1 {
				_, err = writer.Write(comma)
			}
		}
	...
	case TypeString:
		err = escapeStringToWriter(writer, v.s)
	case TypeNumber:
		_, err = writer.Write([]byte(v.s))
	case TypeTrue:
		_, err = writer.Write(trueLiteral)
	case TypeFalse:
		_, err = writer.Write(falseLiteral)
	case TypeNull:
		_, err = writer.Write(null)
	default:
		return fmt.Errorf("unexpected Value type: %d", v.t)
	}
