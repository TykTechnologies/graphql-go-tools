...
func TestImportAsyncAPIDocumentString_EventWrapper(t *testing.T) {
	asyncapiDoc, err := os.ReadFile("./fixtures/joke-service-2.2.0.yaml")
	require.NoError(t, err)
	doc, report := ImportAsyncAPIDocumentString(string(asyncapiDoc))
	if report.HasErrors() {
		t.Fatal(report.Error())
	...
	graphqlDoc, err := os.ReadFile("./fixtures/joke-service-2.2.0.graphql")
	require.NoError(t, err)
	require.Equal(t, string(graphqlDoc), w.String())
}
